          ...
          <div class="control">
            ...
            <div class="select is-medium">
              <select id="openDataWebsite">
                <option  v-for="website in openDataWebsites" :key="website._id" v-bind:value="website.id">{{website.name}}</option>
              </select>
            </div>
          </div>
        </div>
...
export default {
  ...
  setup() {
    const API_URL = 'http://localhost:4242/api/scraper';
    async function getDatasets() {
      const openDataUrl = openDataWebsite.value;
      const fetchOpenDataWebSite = await fetch(API_URL + "/" + searchKeyword.value, 
        {
        method: 'GET',
        })
